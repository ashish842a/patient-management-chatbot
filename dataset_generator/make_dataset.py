...
def generate_patient_data(patient_id, existing_data=None):
    """
    Generates patient data, checking if existing data exists. If so, use it; otherwise, generate new data.
    """
    if existing_data and patient_id in existing_data:
        return existing_data[patient_id]
    ...
    # Format the date as 09-Aug-2024
    formatted_last_visit = last_visit.strftime("%d-%b-%Y")
    ...
    patient_data = {
        ...
        "LastVisit": formatted_last_visit,  # Use formatted date
...
def write_data_in_batches(output_file, batch_size, total_records):
    # Initialize the fieldnames and open the file in append mode
    fieldnames = [
    ...
    # Open the file once and append data
    with open(output_file, mode="a", newline="", encoding="utf-8") as file:
        ...
        for i in range(1, total_records + 1, batch_size):
            batch_data = [generate_patient_data(i + j) for j in range(batch_size) if (i + j) <= total_records]
            writer.writerows(batch_data)
            
            # Print progress and time taken
            print(f"Batch {i // batch_size + 1} of {total_records // batch_size} completed. {len(batch_data)} records written.")
            
            # Optional: Time delay to simulate realistic batch processing
            time.sleep(2)

    print(f"{total_records} detailed patient records written to {output_file}.")

# Define output file and batch size
output_file = r"D:\my_work\Doctor\Patient_management_system\Dataset\patients_record_1k.csv"
batch_size = 1000  # Process 1 lakh records per batch
total_records = 1000 # 1 Crore records

# Start writing data in batches
write_data_in_batches(output_file, batch_size, total_records)
